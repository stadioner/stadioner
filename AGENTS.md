...
- Sales locations are partner network, pickup point (Výdejní místo), and Šalanda taproom (Výčep na Šalandě); the nav item is a hover dropdown to those three targets.
- B2B product configurator lives on `/pro-firmy` (bottles and kegs); the product icon row is a bounded horizontal scroller, not an infinite carousel.
