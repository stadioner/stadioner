...
export default function HistoriePage() {
  ...
  return (
    <section className='timeline-container bg-brand-action relative h-screen overflow-hidden'>
      ...
      {/* Timeline Navigation */}
      <div className='absolute top-1/2 left-4 z-20 -translate-y-1/2 md:left-18'>
        <div className='relative flex h-[500px] flex-col'>
          <div className='bg-brand-primary/30 absolute top-0 bottom-0 left-1/2 w-0.5 -translate-x-1/2 transform' />
          ...
          {timelineData.map((slide, index) => (
            ...
            >
              ...
              <button
                ...
                className={`absolute left-1/2 z-10 h-4 w-4 -translate-x-1/2 rounded-full border-2 transition-all duration-300 ${
                  index === currentSlide ?
                    'bg-brand-primary border-brand-primary scale-125'
                  : 'border-brand-primary/50 hover:border-brand-primary bg-transparent'
                }`}
              >
                {index === currentSlide && (
                  <motion.div
                    className='bg-brand-primary absolute inset-1 rounded-full'
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
              ...
              {/* Year label */}
              <div className='text-brand-primary absolute top-1/2 left-5 hidden -translate-y-1/2 text-sm whitespace-nowrap md:block'>
                {slide.year}
              </div>
            </div>
          ))}
      ...
      <div className='relative z-10 flex h-full items-center'>
        <div className='container mx-auto pr-2 pl-16 md:pl-48'>
          <AnimatePresence mode='wait'>
            <motion.div
              key={currentSlide}
            ...
            >
              <div>
                <motion.div
                  className='text-brand-primary mb-1 text-lg font-medium'
      ...
      <div className='absolute right-8 bottom-8 z-10 flex gap-4'>
        <motion.button
          onClick={prevSlide}
          className='flex h-12 w-12 cursor-pointer items-center justify-center rounded-full bg-white/20 backdrop-blur-sm transition-all duration-200 hover:bg-white/30 disabled:cursor-not-allowed disabled:opacity-50'
      ...
      >
        <div className='flex flex-col items-center text-sm text-white/60'>
          <motion.div
            className='mb-2 h-8 w-0.5 bg-white/30'
